class MatMath(object):
	...
	@staticmethod
	def CalcProjM(	fov, resX, resY, near, far, 
					cropLeft, cropRight, 
					cropBottom, cropTop):
	...
	@staticmethod
	def CalcOrthoProjM(	wWidth, resX, resY, near, far, 
						cropLeft, cropRight, 
						cropBottom, cropTop):
...
class Math(object):
	...
	@staticmethod	
	def StepVal(range, stepSize, value):
		val = math.floor(value / range * stepSize)	
		return val	

	@staticmethod	
	def ExpandVal(low, high, value, format):
	...
	@staticmethod		
	def NormalizeVal(low, high, value):
		return (value - low) / (high - low)

	@staticmethod	
	def RangeVal(oLow, oHigh, nLow, nHigh, value, format):
